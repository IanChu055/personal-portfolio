Markdown
# Hi, I'm Chu Ieng Seng (Ian) 👋
### B.S.Sc. Architectural Studies @ CUHK | Seeking 2026 Summer Internship

---

## 🏛️ Design Philosophy
My architectural approach is grounded in the belief that design is a **dialogue between matter, memory, and place**. I seek to create architecture that grows from what already exists—honoring the traces of the past while fostering renewal and adaptability.

---

## 🎨 Featured Project
![Reuse the Watchtower (Italy)](./cover.jpg)
*Featured: **Reuse the Watchtower (Italy)** — Adaptive reuse of a historical coastal watchtower, extending its life through a dialogue between ancient stone and contemporary glass/steel structures.*

---

## 📂 Key Projects

### 1. Reuse the Watchtower (Italy)
*Adaptive reuse of a historical coastal watchtower, extending its life through a dialogue between ancient stone and contemporary glass/steel structures.*

### 2. Green Nest – Lee Kee Memorial Dispensary Renovation
*Redefining adaptive reuse as a dialogue between historical presence and urban well-being.*

### 3. Nantou Old Town Reconstruction
*Urban renewal rooted in collective memory and historical continuity.*

---

## 🛠️ Technical Toolkit
- **Modeling:** AutoCAD, SketchUp, Rhino
- **Visualization:** Twinmotion
- **Creative Suite:** Adobe Photoshop, Illustrator, InDesign
- **Fabrication:** Physical Model Making, Laser Cutting

---

## 🔗 Portfolios & Connect
[![Portfolio](https://img.shields.io/badge/Portfolio-PDF-blue)](./portfolio-Ian.pdf) 
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue)](https://www.linkedin.com/in/chu-ian-1336663a3/)

---

## 📫 Contact
- **Email:** [YOUR_EMAIL@LINK.COM]
- **Location:** Hong Kong / Macau
- **Research Interests:** Adaptive Reuse | Contextual Design | Urban Renovation

*"Architecture grows from what already exists."*
